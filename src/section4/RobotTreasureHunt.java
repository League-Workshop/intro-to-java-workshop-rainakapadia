...
public class RobotTreasureHunt implements KeyEventDispatcher {
	...
	private void spaceBarWasPressed() {
		...
		int robotXLocation = abc.getX();
		int robotYLocation = abc.getY();
		...
		if (robotXLocation == 720 && robotYLocation == 430) {
			JOptionPane.showMessageDialog(null, " Now that you have found the girl, \n its time to find the parrots.");
			JOptionPane.showMessageDialog(null, "the parrots are near the torch");
		}
		if (robotXLocation == 605 && robotYLocation == 145) {
			JOptionPane.showMessageDialog(null,
					"good job, now its time to look  for the tree house. \n Remeber, it is near the keep out signs  ");
		}
		if (robotXLocation == 255 && robotYLocation == 230) {
			JOptionPane.showMessageDialog(null, "Amazing, thank you for playing");
		}
