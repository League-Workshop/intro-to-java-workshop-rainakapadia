	...
	public static void main(String[] args) {
		...
		while (playerHealth > 0 && dragonHealth > 0) {
			...
			if (answer.equals("kick")) {
				...
				int q = new Random().nextInt(25);
				// -- Subtract that number from the dragon's health variable
				dragonHealth -= q;
				// 11. Find a random number between 0 and 35 and store it in playerDamage
				int w = new Random().nextInt(35);
			...
			if (playerHealth <= 0) {
				...
				if (dragonHealth <= 0) {
					...
					JOptionPane.showMessageDialog(null, "The dragon is dead and you just won a ton of gold!");
				}
			}
			JOptionPane.showMessageDialog(null, " the dragon's health= " + dragonHealth);
			JOptionPane.showMessageDialog(null, " the amount of health lost by ther player= " + (100 - playerHealth));
